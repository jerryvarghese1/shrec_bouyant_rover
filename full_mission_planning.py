...
payload_width = .1 # width of payload (m)
...
moment_arm_thruster = 4.11 # moment arm of thruster (m)
...
dragthrustratio_list = np.linspace(.01, .99, dtr_iter) # ratio of thrust to drag

percent_error = .01
min_burn_time = 2 # (s) shortest duration of time an engine can burn

vel_elbow_frac = .1 # fraction of average coasting velocity to slowdown to
    
max_single_burn_range = 2000 # m 
min_single_burn_range = 50 # m
range_list = np.linspace(min_single_burn_range, max_single_burn_range, range_iter)

time_step = .5 # simulation time step size

d_tol = 1 # tolerance for displacement (m)
v_tol = .02 # (m/s)
theta_tol = .001 # tolerance for theta (rad)

## Calculation ##
max_theta = np.radians(60) #maximum theta
consider_bouyancy_drift = 0

master_kindata_sheet_2d = []

fuel_used_2d = np.zeros((len(dragthrustratio_list)*len(range_list),3))
maneuver_time_2d = np.zeros((len(dragthrustratio_list)*len(range_list),3))

counter = 0

for a in range(len(range_list)):
    target_range = range_list[a]
    print('Range Iteration %d of %d'%(a+1, len(range_list)))
    master_kindata_sheet_1d = []
        
    for b in range(len(dragthrustratio_list)):
        
        dragthrustratio = dragthrustratio_list[b]
        print('Ratio Iteration %d of %d'%(b+1, len(dragthrustratio_list)))
        ...
        vel_elbow = vel_elbow_frac * max_vel
        ind_ignore = np.where(init_data['velocity_x'] > vel_elbow)[0][0]
        ind_at_end = int(maneuver_time/time_step) + 1
        
        f_b_f = np.linspace(ind_ignore/ind_at_end, .99, f_b_f_iter)
        d_list = np.zeros((len(f_b_f)))
        
        
        print('Calculating cutoff time')
        for i in range(len(f_b_f)):
            print('\n  - Cutoff Iteration %d of %d'%(i+1, len(f_b_f)))
        ...
        try:
            ...
            fuel_used = correct_data['fuel_mass'].iloc[-1]
            tot_maneuever_time = correct_data['time'].iloc[-1]
        
            master_kindata_sheet_1d.append(correct_data)
        
            fuel_used_2d[counter,:] = np.array((target_range, dragthrustratio, fuel_mass - fuel_used))
            maneuver_time_2d[counter,:] = np.array((target_range, dragthrustratio, tot_maneuever_time))
        except:
            print('\ntheta is not critically damped or not enough fuel')
            
            master_kindata_sheet_1d.append(0)
            
            fuel_used_2d[counter,:] = np.array((target_range, dragthrustratio, -1))
            maneuver_time_2d[counter,:] = np.array((target_range, dragthrustratio, -1))
            
        print('\n')
        counter = counter + 1
        
    master_kindata_sheet_2d.append(master_kindata_sheet_1d)

fuel_used_interper = interp2d(fuel_used_2d[:, 0], fuel_used_2d[:, 1], fuel_used_2d[:, 2])
maneuver_time_interper = interp2d(maneuver_time_2d[:, 0], maneuver_time_2d[:, 1], maneuver_time_2d[:, 2])
